...
## Propagate Inputs
def propagateInputs(pattern, neurons, weights, function):
    outs = pd.DataFrame([], columns=['Layer','Neuron','Value'])
    
    for layer in range(len(neurons)):
        if (layer+1) < len(neurons):
            for neuron in range(neurons[layer]):
                weights_neuron = weights[(weights['Layer'] == layer + 1)]
            ...
            for i in range(len(nets)):
                nets_sum += np.exp(nets[i])

            for neuron in range(neurons[layer]):
                out = np.exp(nets[neuron]) / nets_sum
                out = pd.DataFrame([[layer + 1, neuron + 1, out]], columns=['Layer','Neuron','Value'])
...
def backpropagateError(neurons, desire, outs, weights, function):
    ...
    output = outs[outs['Layer'] == outs['Layer'].max()]
    for outs_neurons in range(neurons[-1]):
        delta = 0
        out = output[output['Neuron'] == outs_neurons+1].iloc[0,2]
        ...
        while (error >= self.error_target) and (repetitions < self.max_iterations):
            if self.version == 'online':
                ...
                for row in range(self.data.shape[0]):
                    ...
                    #error += calculateError(desire, outs) / data.shape[0]
